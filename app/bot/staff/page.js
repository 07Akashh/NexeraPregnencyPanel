...
function ChatContent() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Save chat to localStorage after first user response
      const chatRecord = {
        id: chatId,
        messages: updated,
        options: res.options || [],
        title: updated.find(m => m.role === "assistant")?.content?.slice(0, 30) || "New Chat",
        date: new Date().toISOString().split('T')[0]
      };
      localStorage.setItem(`chat_${chatId}`, JSON.stringify(chatRecord));
